...
Earlier drafts tried to put the "do I know HOW?" self-check inline into `superpowers:executing-plans`. That pollutes a general-purpose skill with enforcement logic only some users want. Extracting it into its own slash command + skill means:

- Users without the hook installed get untouched `superpowers:executing-plans`.
- Users with the hook installed get a focused, scoped handler they can read end-to-end.
- `superpowers:executing-plans` keeps its short, generic shape and stays easy to reason about.
...
Under context-mode, the `verifyCommand` output may be computed in a ctx sandbox to keep the raw flood out of context — but the `Gate:` / `AC: … PROVEN BY …` lines MUST still be echoed into the transcript (the assistant message), not left only in the sandbox. The hooks scan the transcript; sandbox-only evidence triggers a false-positive block. See `skills/shared/context-mode-adapter.md`.
...
AC: sensor optimization_status = idle — PROVEN BY GET /api/states/sensor.marstek_battery_a3f1_optimization_status → "idle"
...
- Does not replace `superpowers:brainstorming`. Brainstorming explores design; this flow nails down verification mechanics at the latest moment when they matter (execute time).
- Does not modify `superpowers:executing-plans` decision logic. Executing-plans gains one surgical paragraph ("run gates as specified") and nothing else.
...
- End-to-end integration test at `tests/claude-code/test-user-gate-hooks.sh` (10 test cases: fail-open on empty/garbage input, non-gate close pass-through, gate block/pass on `AC:`/`PROVEN BY` evidence, stop-hook block/pass on plan-complete claims, and guard-variable bypass for both hooks; parsing cases skip deterministically when `jq`/`python3` are missing).
