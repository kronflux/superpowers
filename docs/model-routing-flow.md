...
1. **Dispatch notice** (`hooks/pre-agent-model-routing.js`, PreToolUse on Agent): the session's first `Agent` dispatch gets a `<model-routing-active>` block with the project's mapping and the tier rules, injected once per session rather than at every SessionStart. A session that never dispatches a subagent never receives it. `hooks/session-start` still injects a `<model-routing-legacy>` migration offer when the legacy project config path is in use, since that concerns configuration rather than dispatch.
