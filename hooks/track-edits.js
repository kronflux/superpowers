 ...
 * (hooks/lib/gitignore.js), so the "# AI assistant artifacts" section
 * header is written exactly once and coexists with context-engine.js.
 *
 ...
 * Called whenever Claude writes one of these files. Delegates to the
 * shared ensureGitignored helper so the section header is shared with
 * context-engine.js and entries are never duplicated.
 */
function gitignoreArtifact(filePath, cwd) {
  try {
    const basename = path.basename(filePath);
    if (!AI_ARTIFACTS.includes(basename)) return;

    const dir = filePath && path.isAbsolute(filePath) ? path.dirname(filePath) : (cwd || '.');
    ensureGitignored(dir, [basename]);
  } catch {
...
function logEdit(tool, filePath, cwd, sessionId) {
  try {
    ...
    gitignoreArtifact(resolved, cwd);
...
export { logEdit, getRecentEdits, rotateIfNeeded, gitignoreArtifact, EDIT_LOG, LAST_SAVED_FILE, LOG_DIR };
