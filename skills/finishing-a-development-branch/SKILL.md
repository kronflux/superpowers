...
Include a `## Notable Decisions` section (trade-offs, alternatives rejected, non-obvious choices). If `session-log.md` exists and has `[saved]` entries from this branch's lifetime, extract the Decisions and Rejected bullets from the most recent entry and include them there.
