...
4. **If no patch:** Document in `known-issues.md` with the CVE, affected dependency, workaround (if any), and date to re-check (see `superpowers:error-recovery`).
...
- `superpowers:systematic-debugging` — when an update causes unexpected failures
- `superpowers:test-driven-development` — when the update requires new tests for changed behavior
- `superpowers:error-recovery` — to document recurring dependency issues in `known-issues.md`
