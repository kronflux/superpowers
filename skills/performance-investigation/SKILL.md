...
- `superpowers:systematic-debugging` — when performance degradation is actually a bug (infinite loop, memory leak from a logic error)
- `superpowers:test-driven-development` — when the performance fix requires behavior changes
- `superpowers:refactoring` — when the fix is purely structural (reorganizing code for better locality, no behavior change)
