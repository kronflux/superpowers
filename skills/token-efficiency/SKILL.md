...
2. **Verbatim URL content needs the right fetch path.** For verbatim content from URLs (e.g., GitHub raw files, config files, source code), follow `superpowers:skills/shared/conductor/context-mode-adapter.md` — use `ctx_fetch_and_index` then `ctx_search` when context-mode is active (raw page bytes never enter context); use WebFetch or a native fetch when inactive (native fallback). Do not assume WebFetch returns raw text — when context-mode is inactive it may return an AI-generated summary, so request the specific verbatim section explicitly.
...
**Break context by:** invoking `superpowers:context-management` to write `state.md` with discovered facts, then starting fresh with only `state.md` as input. Always save to `state.md` *before* compacting — never after.
...
Use `superpowers:context-management` when cross-session persistence is needed:
...
Within a session, Claude Code handles context compression automatically — do not invoke `superpowers:context-management` just because the session is long.
