...
4. **Verbatim URL content needs the right fetch path.** Fetch as a discovery job — raw page bytes never enter context when ctx tools are used. Do not assume WebFetch returns raw text; when context-mode is inactive it may return an AI-generated summary, so name the verbatim section explicitly.
...
**To break:** invoke `superpowers:context-management` to write `state.md` with discovered facts, then start fresh with only `state.md` as input. Save *before* compacting, never after.
...
Invoke `superpowers:context-management` when state must cross sessions: the user asks to save or compress, work continues in a new session, or accumulated decisions must survive a restart. Within a session Claude Code compresses automatically — do not invoke it merely because the session is long.
