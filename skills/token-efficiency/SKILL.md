...
3. **Verbatim URL content needs the right fetch path.** Fetch as a discovery job — raw page bytes never enter context when ctx tools are used. Do not assume WebFetch returns raw text; when context-mode is inactive it may return an AI-generated summary, so name the verbatim section explicitly.
