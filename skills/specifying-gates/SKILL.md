...
2. The agent ran the "do I know HOW?" self-check (see `superpowers:executing-plans`) and concluded the verification mechanics are ambiguous, OR
...
- Does not close the task. It only enriches metadata; the agent returns to `superpowers:executing-plans` afterward.
...
Use `AskUserQuestion`, one question per call, in order: Q2's answer gates whether the dispatch-brief follow-up is asked, which the batching rule in `${CLAUDE_PLUGIN_ROOT}/skills/shared/output-contract.md` allows for a question whose answer changes what gets asked next.
...
3. **Sync `.tasks.json`.** Update the task entry's description and metadata, set `lastUpdated`.
   ...
   Do NOT route metadata writes through `ctx_execute` — ctx sandboxes discard their FS. Use native TaskUpdate / file write; mutations always stay native per `${CLAUDE_PLUGIN_ROOT}/skills/shared/conductor/routing.md`.

4. **Announce:** "Specification locked. Returning control to executing-plans to run the gate."

## Handoff

Control returns to `superpowers:executing-plans`. The agent reads the updated task, executes `verifyCommand` (or dispatches the subagent with `subagentBrief`), captures output, and posts `AC: <criterion> — PROVEN BY <evidence>` per criterion before closing.

## What NOT to do in this skill

- Do NOT invoke `ExitPlanMode` or `EnterPlanMode`.
- Do NOT start the verification. The verification runs back in `superpowers:executing-plans` after this skill exits.
...
- **Invoked from:** `superpowers:executing-plans` (automatic), `/specify-gate` slash command (manual).
- **Returns to:** `superpowers:executing-plans`.
