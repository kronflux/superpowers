...
Any skill with a data-processing surface (web fetch, build runs, unbounded Bash, analysis reads, count/filter greps) MUST use the canonical sentence — "Tool selection is governed by `${CLAUDE_PLUGIN_ROOT}/skills/shared/conductor/routing.md` — declare the job (discovery / symbol-edit / docs / output / dispatch / memory) and follow its chain." — rather than emitting `curl`/`wget`/`WebFetch`/"Read directly"/"Grep to locate" directives or restating routing.md's per-tool mechanics inline. Those directives are hard-blocked or nudged when context-mode is active; duplicated chains are caught by lint's duplication guard. State-probes, mutations, file writes, and git stay native.
