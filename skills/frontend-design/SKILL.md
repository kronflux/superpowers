...
description: >
  ...
  any React/Vue/Svelte/HTML component work. Routed by superpowers:using-superpowers,
...
See `superpowers:skills/shared/context-mode-adapter.md` — when context-mode is active, inspect these config files via ctx_execute_file and locate them via ctx_execute; otherwise use native Read/Glob.
...
1. **Existing project** — inspect `package.json` and CSS entry files to detect the current framework and major version (ctx_execute_file when context-mode active; native Read otherwise — see `superpowers:skills/shared/context-mode-adapter.md`). Use whatever version is already in use. Do not silently upgrade.
...
This is a LOAD-BEARING native Read of the bundled `tailwind-v4.md` file — its v4 patterns must be in context verbatim to write correct Tailwind code, so it is read with the native Read tool (not routed through ctx, which would keep the reference out of context). Alternatively, when context-mode is active and you need the latest upstream guidance, fetch current Tailwind docs via context7 (`mcp__plugin_context7_context7__query-docs`) — but the bundled `tailwind-v4.md` remains the canonical offline reference.
...
- During `superpowers:executing-plans` or `superpowers:subagent-driven-development` when tasks involve UI/frontend
...
- Before declaring frontend work complete, run the Pre-Delivery Checklist and route review through `superpowers:requesting-code-review`
