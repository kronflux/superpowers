...
Question sequencing follows `${CLAUDE_PLUGIN_ROOT}/skills/shared/output-contract.md`. Mode, Segments, and Separator are independent, but `AskUserQuestion` takes one question per call — ask the three in order below, options defaulted from step 1 on a re-run:
