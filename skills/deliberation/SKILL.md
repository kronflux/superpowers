...
**Do not use for:** decisions that are already well-framed with clear options. Those go directly to `superpowers:brainstorming`. Deliberation is for when you're not yet ready for options.
...
- "the reframe changes the scope — revisit superpowers:premise-check before continuing"
...
- If fewer than 3 genuine perspectives exist for this decision, the decision is probably already well-framed. Use `superpowers:brainstorming` instead.
