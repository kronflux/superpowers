...
1. **Create Failing Test Case**
   ...
   - Use the `.agent/skills/test-driven-development/SKILL.md` skill for writing proper failing tests
...
3. **Verify Fix**
   ...
   - Use the `.agent/skills/verification-before-completion/SKILL.md` skill before claiming success
...
1. **Recurring error?** → offer to add the error→solution mapping to `known-issues.md` (create it if absent) using the format from the `.agent/skills/error-recovery/SKILL.md` skill, if installed.
2. **Permanent constraint discovered?** → offer to add it to `project-map.md` Critical Constraints via `.agent/skills/context-management/SKILL.md`, if that skill is installed and the file exists or the user agrees to create it.
