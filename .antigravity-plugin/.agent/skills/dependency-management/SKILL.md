...
1. **List outdated dependencies:**
   ...
   These produce bounded structured output — discovery job (see Adapter Link above). The same applies to `npm audit`.
...
1. **Read the changelog/migration guide.** Look for:
   ...
   When the changelog is a URL, fetch it as a discovery job (see Adapter Link above).

2. **Search the codebase for usage of changed APIs** — discovery job (see Adapter Link above). For each renamed/removed API, run separate searches for:
