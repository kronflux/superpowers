...
2. Search it for the error message, error code, or failing test name. See `.agent/skills/skills/SKILL.md/shared/context-mode-adapter.md` — when context-mode is active, search via `ctx_search`/`ctx_execute_file` over `known-issues.md`; otherwise native Grep (native fallback).
