...
When the `context-mode` plugin is active (its `ctx_*` MCP tools are present), route data work (analysis reads, count/filter/aggregate greps, unbounded exploratory Bash output) through the ctx tools per `skills/shared/conductor/context-mode-adapter.md`. State-probes, mutations, file writes, and git stay native.
...
1. Call `the task.md task list` to check for existing tasks from brainstorming
2. If tasks exist: you will enhance them with implementation details as you write the plan
3. If no tasks: you will create them with `the task.md task list` as you write each plan task

**Do not proceed to exploration until the task.md task list has been called.**
...
> **For agentic workers:** REQUIRED SUB-SKILL: Use .agent/skills/subagent-driven-development/SKILL.md (recommended) or .agent/skills/executing-plans/SKILL.md to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.
...
Show the opt-in heads-up before the Execution Handoff only if some task has `userGate: true` AND the canonical user-gate hook isn't registered in the merged settings files. Otherwise suppress it; never make it an `ask_question`. Detection command + exact text: `references/user-gate-tagging.md`.

## Execution Handoff

<HARD-GATE>
STOP. You are about to complete the plan. DO NOT call EnterPlanMode or ExitPlanMode. Both are FORBIDDEN — EnterPlanMode traps the session, ExitPlanMode skips the user's execution choice. Do NOT call ask_question either; completion is deterministic per the Selection Logic below.
</HARD-GATE>
...
**If Subagent-Driven:**
- **REQUIRED SUB-SKILL:** Use .agent/skills/subagent-driven-development/SKILL.md
- Fresh subagent per task + two-stage review

**If Inline Execution:**
- **REQUIRED SUB-SKILL:** Use .agent/skills/executing-plans/SKILL.md
...
For each plan task, create a native task. Embed metadata as a `json:metadata` fence at the END of the description — the only way it survives the task.md task list (the `metadata` param is accepted but not returned).
