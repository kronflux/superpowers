...
|---|---|
| read_url_content / `curl`·`wget` to stdout / inline HTTP in scripts | `ctx_fetch_and_index` → `ctx_search`, or `ctx_execute` (read_url_content is hard-denied by context-mode) |
