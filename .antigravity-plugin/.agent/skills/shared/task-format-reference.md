...
Skills that create native tasks (the task.md task list) MUST follow this format.

## Task Description Template

Every the task.md task list description MUST follow this structure:
...
Embed metadata as a `json:metadata` code fence at the end of the the task.md task list description. The `metadata` parameter on the task.md task list is accepted but **not returned by the task.md task list** — embedding in the description is the only reliable way.
...
| `modelTier` | string | no | Abstract capability tier: `"mechanical"` \| `"standard"` \| `"advanced"` \| `"frontier"`. Resolved at dispatch via docs/superpowers/model-routing.json. File absent → inert metadata; every subagent inherits the session model. A concrete `model` pin overrides tier. `advanced` is the default ceiling; `frontier` is gated and costs 2x. |
| `frontierConsent` | string | no | Approval token of the form `FRONTIER-APPROVED:task-<N>`, required alongside `modelTier: "frontier"`. The dispatch gate allows a frontier-model dispatch only when this token ALSO appears in a harness-authored tool_result in the transcript — i.e. the user actually selected the approval option. Writing it without that approval does not satisfy the gate. |
...
Why both the flag and the prose: the prose protects the current session (visible to the task.md task list), the flag protects future hooks and subagents that parse metadata.
...
the task.md task list:
  ...
  description: |
    ...
    **Acceptance Criteria:**
    - [ ] ask_question presents 3 most recent JIT messages
    - [ ] Selected JIT's SOC and schedule are parsed into variables
    - [ ] --jit override bypasses the prompt (backwards compat)

    **Verify:** Read the Pre-flight Step 2 section and confirm ask_question block with 3 JIT options

    ```json:metadata
    {"files": [".claude/commands/hame-optimal-cycle-inspection.md"], "verifyCommand": "grep -A 20 'Step 2' .claude/commands/hame-optimal-cycle-inspection.md", "acceptanceCriteria": ["ask_question with 3 JIT options", "SOC + schedule parsed from selection", "--jit override bypasses prompt"]}
    ```
```
