...
| `modelTier` | string | no | Abstract capability tier: `"mechanical"` \| `"standard"` \| `"frontier"`. Resolved at dispatch via docs/superpowers/model-routing.json. File absent → inert metadata; every subagent inherits the session model. A concrete `model` pin overrides tier. |
