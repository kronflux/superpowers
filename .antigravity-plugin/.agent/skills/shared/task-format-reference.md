...
| `modelTier` | string | no | Abstract capability tier: `"mechanical"` \| `"standard"` \| `"advanced"` \| `"frontier"`. Resolved at dispatch via .superpowers/model-routing.json. File absent → inert metadata; every subagent inherits the session model. A concrete `model` pin overrides tier. `advanced` is the default ceiling; `frontier` is gated and costs 2x. |
