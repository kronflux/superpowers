...
3. **Ask clarifying questions** — understand purpose/constraints/success criteria
...
- Question style (batching, when to ask alone) is governed by `${CLAUDE_PLUGIN_ROOT}/skills/shared/output-contract.md` and is not restated here
...
- `.agent/skills/writing-plans/SKILL.md` is invoked as the next skill.
...
Set up dependency relationships with `the task.md task list(taskId, addBlockedBy:[prerequisite-ids])`.

### Before Handoff

Run `the task.md task list` to display the complete task structure with dependencies.
