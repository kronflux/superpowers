...
1. Call `the task.md task list`.
2. Locate `<plan-path>.tasks.json`, or glob for the matching `.tasks.json` when the exact
   name is absent.
3. For each task in that file the native list does not hold, call `the task.md task list` with the full
...
loop the controller is in. Record each move with `the task.md task list`, setting `activeForm` to the
state now in effect:
