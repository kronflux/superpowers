...
Data-work tool choice defers to `skills/shared/conductor/context-mode-adapter.md` when context-mode is active (direct fetch directives are hard-blocked in that mode); the rules below apply when it is not. When context-mode is active, route fetches through `ctx_fetch_and_index`, file analysis through `ctx_execute_file`, and count/filter searches through `ctx_search`/`ctx_execute` so raw bytes stay out of context; when inactive, use the native tools and read the output (native fallback). Writes, mutations, and state-probes stay native in both modes.
...
3. Match read scope to task type: locate specific known content (a function, a config value, an error handler) with a targeted search; read complete files when the task requires understanding what a file covers (scope assessment, gap analysis, systemic recommendations). Partial reads cannot prove absence. Tool selection per `.agent/skills/skills/SKILL.md/shared/conductor/context-mode-adapter.md` — `ctx_search`/`ctx_execute` for searches and `ctx_execute_file` for whole-file analysis when context-mode is active; native Grep/Read when inactive (native fallback).
...
2. **Verbatim URL content needs the right fetch path.** For verbatim content from URLs (e.g., GitHub raw files, config files, source code), follow `.agent/skills/skills/SKILL.md/shared/conductor/context-mode-adapter.md` — use `ctx_fetch_and_index` then `ctx_search` when context-mode is active (raw page bytes never enter context); use read_url_content or a native fetch when inactive (native fallback). Do not assume read_url_content returns raw text — when context-mode is inactive it may return an AI-generated summary, so request the specific verbatim section explicitly.
3. **Use agents for conclusions, not data relay.** Good: "Analyze the test failures in X and recommend fixes." Bad: "Fetch files A, B, C and return their contents." If you need raw content in your context, pull it in yourself — tool choice per the Adapter Link deferral.
4. **For local files: pull them in yourself, do not relay through an agent.** Do not dispatch an agent to read project files and report back — you lose the actual content and waste tokens on the round trip. Per `.agent/skills/skills/SKILL.md/shared/conductor/context-mode-adapter.md`, use `ctx_execute_file` to analyze the file when context-mode is active; use the native Read tool when inactive (native fallback).
5. **project-map.md is orientation, not understanding.** The map tells you what exists and where — directory purposes, key file roles, constraints. It does NOT contain the logic inside each file. When you need to understand a file's actual implementation (for modification, comparison, or debugging), pull it in directly (`ctx_execute_file` when context-mode is active; native Read when inactive — see the adapter). The map saves you from re-discovering project *structure*; it does not replace reading the files you need to work with.

## Exploration Tracking

Maintain a mental index of repository exploration performed in this session. Before every Read, Grep, Glob, or ctx analysis call, check this index and skip the call if the result is already known and the file has not been modified since.
...
| Identical search pattern + same scope | Do NOT re-run — reuse the previous results |
| Similar but broader search pattern | Run the new search — it may surface new results |
| Context compression occurred (earlier turns disappeared) | Trust your remaining knowledge of file contents; only re-read if you genuinely cannot recall the content you need. When context-mode is active, query auto-captured session memory first via `ctx_search` (`sort:"timeline"`) before re-fetching |

### On Context Compression

When Claude Code compresses earlier messages, you lose the raw tool output but retain your own reasoning and summaries. This does NOT mean you need to re-read everything — you still know what you learned. Only re-read when you need specific content (exact line numbers, precise syntax) that you can no longer recall. When context-mode is active, prior decisions, errors, and captured outputs are searchable via `ctx_search` (`sort:"timeline"`) — query that before re-fetching.
...
**Break context by:** invoking `.agent/skills/context-management/SKILL.md` to write `state.md` with discovered facts, then starting fresh with only `state.md` as input. Always save to `state.md` *before* compacting — never after.
...
Use `.agent/skills/context-management/SKILL.md` when cross-session persistence is needed:
...
Within a session, Claude Code handles context compression automatically — do not invoke `.agent/skills/context-management/SKILL.md` just because the session is long.
...
When context-mode is active, its routing and sandbox handle output reduction directly — noisy commands run through `ctx_batch_execute`/`ctx_execute`, only derived results enter context, and the bash-compress hook yields to that routing. The guidance in this section applies when context-mode is **inactive**: the plugin's smart-compress hook then compresses noisy Bash output before it enters your context. When you see a line like `[compressed: 120->4 lines | git-status]`, the output was filtered to remove noise while preserving signal.
...
- File reads — content was explicitly requested
- Commands with user-applied pipes (`| grep`, `| awk`, `| sed`)
- Commands with `--verbose` or `--debug` flags
- HTTP responses — API output should not be truncated
...
- Re-running the same search you already ran this session
