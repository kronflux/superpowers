...
User-gate enforcement is an **opt-in flow**. When the opt-in hook is not registered, `.agent/skills/executing-plans/SKILL.md` runs unchanged — no extra checks, no extra context, no extra questions. When the hook IS registered, it routes user-gate tasks through this skill. Keeping the decision logic in a separate skill means:

- Users who don't want the flow get zero friction.
- Users who do want it get a focused, scoped handler.
- `.agent/skills/executing-plans/SKILL.md` stays short and readable.
...
1. `the task.md task list <task-id>` — read the full description.
...
4. If every criterion passed → `the task.md task list status=completed`.
...
- Do NOT modify `.agent/skills/executing-plans/SKILL.md`' behavior from inside this skill. This skill is a leaf — it returns control when done.
