...
**Step 2 — Search each keyword individually first.** Per `.agent/skills/skills/SKILL.md/shared/context-mode-adapter.md`, route the search via `ctx_search`/`ctx_execute_file` over `session-log.md` when context-mode is active; when inactive, use native grep (native fallback):
```bash
