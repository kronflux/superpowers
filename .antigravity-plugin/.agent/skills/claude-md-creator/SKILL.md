...
1. **Scan the repo** — read key config files (`package.json`, `tsconfig.json`, `Makefile`, CI configs) and source structure. See `.agent/skills/skills/SKILL.md/shared/context-mode-adapter.md` — when context-mode is active, inspect these via `ctx_execute_file`; otherwise use native Read (native fallback).
