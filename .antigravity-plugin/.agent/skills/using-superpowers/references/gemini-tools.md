...
|-----------------|----------------------|
| `view_file` (file reading) | `read_file` |
| `write_to_file` (file creation) | `write_file` |
| `replace_file_content` (file editing) | `replace` |
| `run_command` (run commands) | `run_shell_command` |
| `grep_search` (search file content) | `grep_search` |
| `find_by_name` (search files by name) | `glob` |
| `the task.md task list` (task tracking) | `write_todos` |
...
|-------------------|----------------------|
| `Task tool (.agent/skills/implementer/SKILL.md)` | `@generalist` with the filled `implementer-prompt.md` template |
| `Task tool (.agent/skills/task-reviewer/SKILL.md)` | `@generalist` with the filled `task-reviewer-prompt.md` template (spec compliance + code quality in one review) |
| `Task tool (.agent/skills/code-reviewer/SKILL.md)` | `@code-reviewer` (bundled agent) or `@generalist` with the filled review prompt |
