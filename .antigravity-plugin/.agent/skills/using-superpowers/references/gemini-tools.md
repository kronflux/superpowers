...
Skills dispatch with `Subagent (general-purpose):` and either reference a prompt-template file (e.g., `.agent/skills/subagent-driven-development/SKILL.md`'s `./implementer-prompt.md`) or supply an inline prompt. On Gemini CLI:
...
| References `.agent/skills/requesting-code-review/SKILL.md`'s `./code-reviewer.md` | `invoke_agent` with `agent_name: "generalist"` and the filled review template |
