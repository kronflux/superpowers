...
Read the changed files using the native `Read` tool. When context-mode is active and you need to analyze file contents at scale rather than inspect exact bytes, use `ctx_execute_file` for the read. Then systematically try to break the code by thinking like an attacker who has full knowledge of the implementation.
...
The `## Summary` section and the ASI score are **MANDATORY** — a report without them is malformed.
...
**Read-only. Output only.** Produce your Breakage Report as text in this conversation. Do NOT write files to disk. Do NOT execute code. Do NOT run shell commands. Do NOT use `ctx_execute` (it runs code) — only `ctx_execute_file` is permitted, and only for reading/analyzing file contents, never for execution side effects. Test case skeletons belong inside markdown code blocks in your report — they are documentation, not files to create.
